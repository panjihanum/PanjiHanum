...
<div id="content">
  <div id="content-header">
    ...
    <h1>Insert Penduduk</h1>
    </div>
    <div class="container-fluid">
      <?php
        if(isset($_GET['pesan']))
        {
          if($_GET['pesan']== "gagalInsert")
          {
            echo"Insert Data Gagal, Data sudah ada";
          }
          else
          {
            ...
            <div class='widget-content nopadding'>
              <form class='form-horizontal' method='post' action='ControllerPenduduk.php' name='add_users_validate' id='add_users_validate' novalidate='novalidate'>
                ...
                <div class='control-group'>
                  ...
                  <div class='controls'>
                    <input type='text' name='name' id='nama_users'>
                  </div>
                </div>
                <div class='control-group'>
                    ...
                    <select name="regions">
                      <option value="0"> please select </option>
                        <?php 
                          ...
                          $get = mysqli_query($koneksi, "SELECT id FROM regions ORDER BY id ASC");
                          while($row = mysqli_fetch_assoc($get))
                          {
                            $rows = $row['id'];
                            $getdaerah = mysqli_query($koneksi, "SELECT name FROM regions WHERE id='$rows'");
                            $array = mysqli_fetch_array($getdaerah);
                        ?>

                          <option value="<?php echo($row['id']) ?>"><?php echo($array['name']) ?></option>
                ...
                <div class='control-group'>
                  ...
                  <div class='controls'>
                    <input type='text' name='alamat' id='nama_users'>
                  </div>
                </div>
                <div class='control-group'>
                  <label class='control-label'>Gaji</label>
                  <div class='controls'>
                    <input type='text' name='gaji' id='nama_users'>
                  </div>
                </div>
              <div class='form-actions'>
                <input type='submit' value='Insert' class='btn btn-success' name='submitInsert'>
              </div>
            </form>
          </div>
        </div>
